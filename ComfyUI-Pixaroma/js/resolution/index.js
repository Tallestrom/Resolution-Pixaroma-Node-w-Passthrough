...
function injectCSS() {
  ...
  const css = `
    ...
    .pix-res-root.passthrough-active .pix-res-chips,
    .pix-res-root.passthrough-active .pix-res-list {
      opacity: 0.42;
      filter: grayscale(1);
      pointer-events: none;
    }
    .pix-res-root.passthrough-active .pix-res-chip.active,
    .pix-res-root.passthrough-active .pix-res-row.active,
    .pix-res-root.passthrough-active .pix-res-snap-btn.active,
    .pix-res-root.passthrough-active .pix-res-quickpick.active {
      background: #343434;
      border-color: #555;
      color: #aaa;
    }
    .pix-res-root.passthrough-active .pix-res-chip.active .pix-res-chip-shape {
      background: rgba(170, 170, 170, 0.22);
      border-color: #777;
    }
    .pix-res-root.passthrough-active .accent,
    .pix-res-root.passthrough-active .pix-res-preview-label .accent {
      color: #aaa;
    }
    .pix-res-root.passthrough-active .pix-res-preview-rect {
    ...
    .pix-res-toggle:hover { border-color: #777; }
    .pix-res-toggle.on {
      background: rgba(246,103,68,0.25);
      border-color: ${BRAND};
    }
    .pix-res-toggle.on::before {
...
//
// NODE_H sized so the passthrough row plus all 8 preset rows fit inside
// the locked node bounds. Width/height input ports add extra ComfyUI chrome
// above the DOM widget, so keep the widget height based on UI_CHROME_H rather
// than the old no-input-port estimate.
const NODE_W = 240;
const NODE_H = 560;
const UI_CHROME_H = 104;
...
const PASSTHROUGH_PROP = "passthrough";
const PASSTHROUGH_INPUT_NAME = "passthrough"; // matches Python INPUT_TYPES key
...
function readPassthrough(node) {
  // Prefer node.properties because it serializes cleanly with the custom DOM UI.
  // Fall back to the normal Python-created BOOLEAN widget so older workflows
  // saved before this JS-side switch keep their value after reload.
  if (Object.prototype.hasOwnProperty.call(node.properties || {}, PASSTHROUGH_PROP)) {
    return coerceBool(node.properties[PASSTHROUGH_PROP], false);
  }
  const widget = findWidget(node, PASSTHROUGH_INPUT_NAME);
  return coerceBool(widget?.value, false);
}

function writePassthrough(node, value) {
  const v = !!value;
  if (!node.properties) node.properties = {};
  node.properties[PASSTHROUGH_PROP] = v;

  // Keep the hidden/native widget value in sync for workflows/frontends that
  // still serialize required widget values independently of properties.
  const widget = findWidget(node, PASSTHROUGH_INPUT_NAME);
...
function renderPassthroughToggle(node) {
  const enabled = readPassthrough(node);

  const wrap = document.createElement("div");
  wrap.className = "pix-res-passthrough";

  const label = document.createElement("span");
  label.className = "pix-res-passthrough-label";
  label.textContent = "Input Passthrough";

  const toggle = document.createElement("button");
  toggle.type = "button";
  toggle.className = "pix-res-toggle" + (enabled ? " on" : "");
  toggle.setAttribute("aria-label", "Input Passthrough");
  toggle.setAttribute("aria-pressed", enabled ? "true" : "false");
  toggle.title = enabled
    ? "Passthrough on: connected width and height inputs are returned unchanged"
    : "Passthrough off: use the selected resolution from this node";

  toggle.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    writePassthrough(node, !readPassthrough(node));
    renderUI(node);
  });
...
function renderUI(node) {
  ...
  root.classList.toggle("passthrough-active", readPassthrough(node));
  root.innerHTML = "";
  root.appendChild(renderPassthroughToggle(node));
...
function setupResolutionNode(node) {
  ...
  hideWidget(node, PASSTHROUGH_INPUT_NAME);
  ...
  const WIDGET_H = NODE_H - UI_CHROME_H; // keep in sync with the chrome estimate in NODE_H comment
...
app.graphToPrompt = async function (...args) {
  ...
  if (out) {
    ...
    for (const id in out) {
      ...
      entry.inputs[PASSTHROUGH_INPUT_NAME] = node ? readPassthrough(node) : false;
    }
  }
  return result;
};
